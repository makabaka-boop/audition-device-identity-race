...
录制现场可随时热插拔摄像头/麦克风。初次载入、`devicechange`、以及每次回到空闲（成片收尾、授权中取消、开拍失败）都会发起设备枚举，遵循：

- **清单只反映最后一次有效设备事件**：枚举是异步的，每次发起递增代次，只有最新一次的结果允许落地；较早发起的枚举若在较新的设备变更之后才返回，其旧清单直接丢弃——下拉框不会被旧清单覆盖，刚接入的设备不会消失，已手动选定的新设备也不会被退回（否则下一次开拍的精确设备请求会落到非预期输入，或因旧设备已拔除而失败）。
- **空闲时的明确选择在设备仍可用时保持**：重新枚举只在「所选设备确实消失」时才回退到该类首个可用设备（无可用设备则为系统默认），并明确提示「所选摄像头「X」已断开，下一次录制将使用「Y」」；手动改选或开拍后提示消除。
- **授权等待/录制/暂停期间不改写待选设备**：设备变更只刷新清单；界面展示的设备以开拍瞬间冻结的采集计划为准（内核 `getActivePlan`），与本次实际采集设备始终一致——冻结设备中途断开时以「（已断开）」占位项继续显示原设备名，绝不显示与已冻结计划不一致的名称。回到空闲（收尾/取消/失败）时才按最新清单对齐待选设备，因此停止后再开一条不会沿用被热插拔替换的选择而录到错误设备。
- **下一条成片的设备身份可预期**：停止后再开拍沿用空闲时确认的选择；所选设备消失时，提示中给出的即下一次录制实际将使用的设备。
...
- **设备热插拔与枚举时序**（可控枚举返回顺序 + devicechange 插拔 + 可控取流等待，hook + UI 两层）：迟到的旧枚举不覆盖新清单、不退回手动选择；空闲明确选择在设备可用时保持、消失时回退并明确提示下一次录制将使用的设备；授权等待/录制/暂停期间设备变更只刷新清单不改写待选，下拉框展示冻结计划中的实际采集设备（断开时占位「已断开」）；取流约束、当前与下一条成片的设备身份、旧成片与交付选择逐项核对。
...
src/
  ...
  hooks/useAuditionRecorder.ts    # 设备枚举（代次守卫/选择保持/回退提示）、take/URL 内存管理、预览复用单流
